...
class TenantService(
    ...
    val parent = args.parentId?.let {
      tenantRepository.findById(it)?.apply {
        if (this.platform != platform) {
          log.info("新增租户失败, 因为父租户 [{} {}] 不属于传入的平台: {}", it, this.name, platform)
          throw BadRequestException("父租户所属平台如传入平台不一致")
        }
      } ?: run {
        log.info("新增租户失败, 父租户: {} 不存在", it)
  ...
  suspend fun freeze(id: Long) {
    tenantRepository.findById(id)?.let {
      if (it.isFrozen) {
        log.info("租户: [{} {}] 当前已是冻结状态", id, it.name)
        return
      }
      it.freeze()
      tenantRepository.save(it)
      log.info("成功冻结租户: [{} {}]", id, it.name)
    } ?: let {
  ...
  suspend fun unfreeze(id: Long) {
    tenantRepository.findById(id)?.let {
      if (!it.isFrozen) {
        log.info("租户: [{} {}] 当前非冻结状态", id, it.name)
        return
      }
      it.unfreeze()
      tenantRepository.save(it)
      log.info("成功解冻租户: [{} {}]", id, it.name)
    } ?: let {
      throw ResourceNotFoundException("冻结失败, 租户信息不存在")
    }
  }
