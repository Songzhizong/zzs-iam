...
class UserAffService(
  ...
  suspend fun addPlatformUser(
    ...
    val platformDo = platformRepository.findByCode(platform) ?: let {
      log.info("添加用户失败, 平台: {} 不存在", platform)
  ...
  suspend fun removePlatformUsers(platform: String, userIds: Set<String>) {
    val platformDo = platformRepository.findByCode(platform) ?: let {
      log.info("移除用户失败, 平台: {} 不存在", platform)
    ...
    if (platformDo.isMultiTenant) {
      if (tenantUserRepository.existsByPlatformAndUserIdIn(platform, userIds)) {
        log.info("从 {}平台移除用户失败, 因为用户尚存在某个租户下", platform)
    ...
    log.info("成功从 {}平台下移除用户数据 {}条", platform, users.size)
  }
  ...
  suspend fun addTenantUser(
    ...
    val tenantDo = tenantRepository.findById(tenantId) ?: let {
      log.info("添加用户失败, 租户 {} 不存在", tenantId)
      throw ResourceNotFoundException("添加用户失败, 租户不存在")
    }
    if (platform != tenantDo.platform) {
      log.info(
        "添加用户失败, 租户 [{} {}] 不属于平台: {}", tenantId, tenantDo.name, platform
      )
  ...
  suspend fun removeTenantUsers(tenantId: Long, userIds: Set<String>) {
    ...
    log.info("成功从租户: {} 下移除用户数据 {}条", tenantId, users.size)
  ...
  /** 获取用户所在的租户列表 */
  suspend fun userTenants(platform: String, userId: String): List<com.zzs.iam.server.dto.resp.UserTenantInfo> {
    ...
    return tenantDos.map { tenant ->
      com.zzs.iam.server.dto.resp.UserTenantInfo().also {
    ...
    return userAvailableInTenantCache.get(key) {
      return@get coroutineScope {
        ...
        if (tenantDo == null || tenantDo.isFrozen) {
          log.info("用户 {} 无法访问租户 {}, 因为租户不存在或者已被冻结", userId, tenantId)
          return@coroutineScope "0"
        }
  ...
  suspend fun changeRoles(
    ...
    if (platformDo.isMultiTenant) {
      if (tenantId == null) {
        log.info("为多租户平台用户分配角色失败, 租户ID为空")
        throw MissTenantIdException()
      }
      val users = tenantUserRepository.findByTenantIdAndUserIdIn(tenantId, userIds)
      if (users.isEmpty()) {
        log.info("获取租户下用户信息列表为空")
  ...
  suspend fun syncAccount(userId: String, account: String) {
    ...
    if (c1 > 0) {
      log.info("用户 {} 的租户用户账号 {}条", userId, c1)
    }
    if (c2 > 0) {
      log.info("用户 {} 的平台用户账号 {}条", userId, c2)
    }
  }
  ...
  suspend fun syncPhone(userId: String, phone: String) {
    ...
    if (c1 > 0) {
      log.info("同步用户 {} 的租户用户手机号 {}条", userId, c1)
    }
    if (c2 > 0) {
      log.info("同步用户 {} 的平台用户手机号 {}条", userId, c2)
    }
  }
  ...
  suspend fun syncEmail(userId: String, email: String) {
    ...
    if (c1 > 0) {
      log.info("同步用户 {} 的租户用户邮箱 {}条", userId, c1)
    }
    if (c2 > 0) {
      log.info("同步用户 {} 的平台用户邮箱 {}条", userId, c2)
    }
  }
    ...
    coroutineScope {
      val async1 = async {
        ...
        if (tenantUsers.isNotEmpty()) {
          ...
          log.info("同步用户 {} 的租户用户信息 {}条", userId, tenantUsers.size)
        }
      }
      val async2 = async {
        ...
        if (platformUsers.isNotEmpty()) {
          ...
          log.info("同步用户 {} 的平台用户信息 {}条", userId, platformUsers.size)
        }
      }
      async1.await()
      async2.await()
